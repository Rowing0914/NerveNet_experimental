...
# ------------------------------------------------------------------------------
import network.init_path as init_path
    ...
    def _build_network_weights(self):
        ...
        with tf.variable_scope(self._name_scope):
            ...
            if 'noninput' not in self._gnn_embedding_option:
                self._MLP_embedding = {
                    node_type: nn.MLP(
                        [self._input_feat_dim / 2, self._node_info['para_size_dict'][node_type]],
                ...
                self._MLP_embedding.update({
                    node_type: nn.MLP(
                        [self._input_feat_dim, self._node_info['para_size_dict'][node_type]],
                        init_method=self._init_method,
                        act_func=['tanh'] * 1,  # one layer at most
                        add_bias=True,
                        scope='MLP_embedding_node_type_{}'.format(node_type)
    ...
    def _build_network_graph(self):
        ...
        if 'noninput' not in self._gnn_embedding_option:
            self._input_embedding = {
                node_type: self._MLP_embedding[node_type](self._input_parameters[node_type])[-1]
                for node_type in self._node_info['node_type_dict']
            }
        else:
            self._input_embedding = {
