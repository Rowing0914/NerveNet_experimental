...
"""
==============================
Norio Kosaka Implemented below
==============================
"""


def readXML(env_name):
    """
    Open and Load the XML

    :return
        - Beautifulsoup xml parer
        - ET xml parser
    """
    # prep the path to the target XML file
    xml_path = os.path.join(XML_ASSERT_DIR, XML_DICT[env_name])
    infile = open(xml_path, 'r')
    xml_soup = bs(infile.read(), 'xml')

    import xml.etree.ElementTree as ET
    xml_tree = ET.parse(xml_path)
    return xml_soup, xml_tree


def get_all_joint_names(env_name):
    """ Get the all joint names """
    # load the target XML file
    xml_soup, _ = readXML(env_name)

    # find the names of joints/bodies
    joints = xml_soup.find('worldbody').find_all('joint')
    name_list = [joint['name'] for joint in joints]
    return name_list


# this is not used!!
def _depth_iter(element, tag=None):
    """ https://stackoverflow.com/a/41536162 """
    stack = []
    ...
    while stack:
        ...
        if e == None:
            ...
            stack.append(iter(e))
            if (tag == None or e.tag == tag) and "name" in e.attrib:
                result.append((e.attrib["name"], len(stack) - 1))
    return result


# this is not used!!
def depth_checker(element, tag="joint"):
    """ Find the deepest elemet with the tag and returns its depth """
    _dic = _depth_iter(element, tag)
    return len(set([depth for name, depth in _dic])) - 1  # return the depth


# TODO: this is not working in the end.....
def get_relationship_joints_nodes(env_name):
    # load the target XML file
    xml_soup, xml_tree = readXML(env_name)
    # depth = depth_checker(xml_tree.getroot(), tag="joint")
    # print(depth)
    # asdf

    # find the names of joints/bodies
    bodies = xml_soup.find('worldbody').find_all('body')

    temp = dict()
    body_cnt = 0

    for body in bodies:
        res = body.find_all('joint')
        joint_list = [x['name'] for x in res]
        if (joint_list[0].find("root") == -1) and (joint_list[0].find("body") == -1):
            # if this node is not the root or body-related, then just append the list below
            temp[joint_list[0]] = joint_list[1:]
        else:
            # if this node is the root or body-related, we need to remove the direct connection to the ankles
            joint_list = [elem for elem in joint_list if elem.find("ankle") == -1]
            t = [elem for elem in joint_list if elem.find("body") != -1]  # Ex => ['body_1', 'bodyupdown_1']
            if len(t) == 2:
                temp[joint_list[0]] = joint_list[1:]
            else:
                body_cnt += 1
                # remove the unnecessary connection from the root or body-related nodes to the next body-related nodes
                remove_list = [elem for elem in t if elem != "body_{}".format(body_cnt)]
                remove_list = [elem for elem in remove_list if elem != "bodyupdown_{}".format(body_cnt)]
                joint_list = [elem for elem in joint_list if elem not in remove_list]
                temp[joint_list[0]] = joint_list[1:]
    print(temp)
    # asdf
...
def _dict_to_graph(g, _dict):
    """ Converrt the dict into a graph """
...
def _construct_kinematics_graph(num_legs):
    """ Construct the kinematics graph for Centipede """
    assert num_legs % 2 == 0, "Please set the even number to num_legs"

    g = nx.Graph()

    # this main part has been fixed across any form of Centipede
    main = {'root': ['lefthip_0', 'righthip_1', 'body_1', 'bodyupdown_1'],
            'lefthip_0': ['ankle_0'],
            'ankle_0': [],
            'righthip_1': ['ankle_1'],
            'ankle_1': []
    ...
    for i in range(int((num_legs - 2) / 2) + 1):
        if i == 0:
            g = _dict_to_graph(g, main)
        else:
            # Add the corresponding legs and its bodies by manually define the body frame in dict
            if i == 1:
                repeat = {'body_%d' % i: ['bodyupdown_%d' % i, 'lefthip_%d' % (2 * i), 'righthip_%d' % (2 * i + 1)],
                          'lefthip_%d' % (2 * i): ['ankle_%d' % (2 * i)],
                          'ankle_%d' % (2 * i): [],
                          'righthip_%d' % (2 * i + 1): ['ankle_%d' % (2 * i + 1)],
                          'ankle_%d' % (2 * i + 1): []
                          }
            else:
                repeat = {'body_%d' % i: ['body_%d' % (i - 1), 'bodyupdown_%d' % (i-1), 'bodyupdown_%d' % i, 'lefthip_%d' % (2 * i),
                                          'righthip_%d' % (2 * i + 1)],
                          'lefthip_%d' % (2 * i): ['ankle_%d' % (2 * i)],
                          'ankle_%d' % (2 * i): [],
                          'righthip_%d' % (2 * i + 1): ['ankle_%d' % (2 * i + 1)],
            ...
            g = _dict_to_graph(g, repeat)
    return g


def _construct_kinematics_graph_Ant():
    """ Construct the kinematics graph for Centipede """
    g = nx.Graph()

    # this main part has been fixed across any form of Centipede
    main = {'root': ['hip_1', 'hip_2', 'hip_3', 'hip_4'],
            'hip_1': ['ankle_1'],
            'ankle_1': [],
            'hip_2': ['ankle_2'],
            'ankle_2': [],
...
def get_adjacency_matrix_Ant(visualise=False):
    graph = _construct_kinematics_graph_Ant()
    if visualise:
        import matplotlib.pylab as plt
        nx.draw(graph, with_labels=True)
        plt.show()
    # the adjacency_matrix doesn't contain the row representing "root"
    # because it isn't the actuator or anything, just a virtual node.
    # Also, we have already removed the observation of "root" from centipede_env.py so that this is okay
    return nx.adjacency_matrix(graph).todense()[1:, 1:].astype(np.float32)

def get_adjacency_matrix(num_legs, visualise=False):
    graph = _construct_kinematics_graph(num_legs)
    if visualise:
        import matplotlib.pylab as plt
        nx.draw(graph, with_labels=True)
        plt.show()
